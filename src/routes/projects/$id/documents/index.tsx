  ...
  return (
    <>
      <SubpageHeader
        project={project}
        title="Документация"
        description="Загруженные документы, их обработка и извлечённые позиции. Клик по документу открывает проверку."
        meta={
          <span className="text-caption text-text-secondary">
            Документов <b className="tnum font-semibold text-text-primary">{documents.length}</b> ·
            извлечено позиций{" "}
            <b className="tnum font-semibold text-text-primary">{fmtNum(totals.extracted)}</b> ·
            проверено{" "}
            <b className="tnum font-semibold text-text-primary">{fmtNum(totals.verified)}</b>
          </span>
        }
        actions={
          <Button
                      ...
                      >
                        <td className="px-4 py-2.5">
                          <div className="flex min-w-0 items-center gap-2.5">
                            <Icon className="size-4 shrink-0 text-text-muted" strokeWidth={1.5} />
                            <div className="min-w-0">
                              <div className="max-w-[360px] truncate font-medium text-text-primary group-hover:text-accent">
                                {doc.title}
                              </div>
              ...
              {rows.map((doc) => {
                ...
                return (
                  <li key={doc.id}>
                    ...
                    >
                      <Icon className="mt-0.5 size-4 shrink-0 text-text-muted" strokeWidth={1.5} />
