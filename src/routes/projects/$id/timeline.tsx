...
import { fmtDateTime, fmtDayTitle, fmtNum, fmtTime } from "@/lib/format";
...
const typeStyle: Record<TimelineEventType, { icon: LucideIcon; tone: string }> = {
  ...
  offer_received: { icon: PackageCheck, tone: "bg-info-bg text-info" },
...
function TimelinePage({ project }: ProjectPageProps): React.JSX.Element {
  ...
  const lastWeek = events.filter(
    (event) => Date.parse(event.at) > Date.parse(now) - 7 * 24 * 3600 * 1000,
  ).length;
  ...
  return (
    <>
      ...
      <div data-main-zone className="grid grid-cols-12 gap-4">
        <section className="card-surface col-span-12 overflow-hidden xl:col-span-7">
        ...
        <div className="col-span-12 space-y-4 xl:col-span-5">
          <WidgetCard>
            <WidgetCardHeader
              ...
              title="Решения"
              hint="с основанием"
              aside={<CountPill>{decisions.length}</CountPill>}
            />
            {decisions.length ? (
              ...
              <p className="text-[13px] text-text-2">Решений по объекту пока нет.</p>
            )}
          </WidgetCard>

          <WidgetCard>
            <WidgetCardHeader icon={History} title="Сводка" hint="по истории" />
            <StatList
              items={[
                { label: "Событий всего", value: fmtNum(events.length) },
                { label: "Показано по фильтрам", value: fmtNum(filtered.length) },
                { label: "Решений", value: fmtNum(decisions.length) },
                { label: "За последние 7 дней", value: fmtNum(lastWeek) },
              ]}
            />
          </WidgetCard>
        </div>
        ...
        {lastDecision && (
          <InsightBlock
            ...
            text={`${lastDecision.reason} · ${employeeById(lastDecision.approvedBy)?.name ?? "—"}, ${fmtDateTime(lastDecision.approvedAt)}`}
          />
        )}
      </div>
