...
| Отчёт с площадки: адаптер не проверяет переход статуса — повторный «вернуть» уже возвращённого отчёта проходит с 200. Найдено проверкой прав на сервере                                                                                    | 0,5 ч  | Правило перехода отчёта (`reportStatus` transitions) есть в контракте, но `reviewReport` его не применяет; правка — в Q6 вместе с тестами правил       |
