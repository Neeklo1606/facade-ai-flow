...
| S4. Полный обход кликабельности     | закрыта: 5 934 нажатия в двух темах, 860 проверок на пяти ширинах, две находки. Перезапущен 22.09 после проверки ветки — см. ниже |
| S5. Справка и инструкция            | закрыта: раздел /help, подсказки со ссылкой в статью, три документа внедрения                                                     |
...
## Решения владельца по итогам S1–S5 (2026-09-22)

Два вопроса, которые аудит S1 оставил владельцу, закрыты его решением; правила записаны
в [ADR-020](docs/adr/ADR-020-name-and-alternatives.md).

| Решение                                                                                   | Состояние                                                                          |
| ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Имя продукта — neeklo FieldOps, «Фасады» — отраслевой пакет; ранние имена убрать отовсюду | закрыто: README, шлюз, превью ссылки, манифест, меню, документы; проверка в тестах |
| Сравнение с Exon, Билдокс, ЦУС и 1С:УСО — по пяти проверяемым параметрам, без цен         | закрыто: данные с источниками, статья справки, раздел в сценарии показа            |

Границу «видит человек» провели по [ADR-020](docs/adr/ADR-020-name-and-alternatives.md), п. 3:
ключи cookie `fieldops_demo` и `fieldops_session`, ключи браузерного хранилища `neeklo-fieldops-*`
и домен демонстрации остались прежними — их переименование сломало бы живые сессии и сохранённое
состояние, не дав покупателю ничего.

## Независимая проверка ветки sales-ready (2026-09-22)

Ветка `feature/sales-ready` против `main` отдана на проверку в отдельную сессию: постановка
S1–S5, решения владельца, ADR-017…020 и полный дифф, без объяснений исполнителя. Рецензент
прогнал конвейер сам, проверил тесты от противного шестью мутациями, обошёл справку за каждую
из пяти персон и открыл источники сравнения своими руками. Два HIGH заблокировали слияние.

| Уровень | Находка                                                                                                                                                      | Как закрыто                                                                                                                              |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| HIGH    | Ссылка «Открыть экран» в справке вела три роли из пяти в отказ по правам: 16 случаев. Хуже всего — статья «Как начать работу», помеченная как статья прораба | `StepTarget` рисует ссылку, только если роль может открыть экран; иначе на её месте причина и перечень ролей. Шаг остаётся видимым       |
| HIGH    | Отчёт обхода S4 и STATE.md утверждали «тупиков нет», хотя обход прогнан до появления экрана справки и после S5 не перезапускался                             | обход перезапущен на текущем коде в обеих темах, отчёт и контактные листы пересобраны                                                    |
| MEDIUM  | Даже после перезапуска обход нашёл бы 3 тупика из 16: статьи переключались состоянием, и до непервой статьи обход не доходил                                 | статья живёт в адресе (`/help?article=…`), список статей — ссылки; в шаблонах обхода `article` со значением, поэтому статья — свой экран |
| MEDIUM  | Светлая тема не держала WCAG AA там, где комментарий это утверждал: `--ok #177A41` давал 4,37:1 на `--surface-3`; светлый прогон не видел ни одного диалога  | `--ok` стал `#166F3B` (5,05:1); добавлен `e2e/dialogs.spec.ts` — четыре окна под axe и правилом одного оранжевого в обеих темах          |
| MEDIUM  | «На странице решения предложено заказать внедрение у партнёра» — на указанной странице 1С такого нет                                                         | утверждение убрано; осталось проверяемое: снятие с продаж 01.07.2019 и замена на 1С:ERP УСО 2, срок — «не проверено»                     |
| MEDIUM  | «Облачный сервис» про Билдокс — сайт себя так не называет                                                                                                    | строка переписана: формат размещения и срок внедрения — «не проверено»                                                                   |
| MEDIUM  | Наша строка сравнения обещала разбор документа без пометки демонстрации, хотя соседняя строка пометку имела                                                  | пометка добавлена: в демонстрации файл не распознаётся, добавляется образец                                                              |
| LOW     | `tests/domain/help.test.ts` не падал при подмене объекта на несуществующий: `screenFor` разбирает адрес по форме                                             | добавлена проверка по данным демо-адаптера; поломка ловится                                                                              |
| LOW     | STATE.md писал «восемь статей», статей девять                                                                                                                | исправлено                                                                                                                               |

Отдельно: правило одного оранжевого при открытом окне считает пятна внутри окна. Страницу
закрывает затемнение `bg-black/80` на z-50 при шапке z-30 — пиксель под оранжевой кнопкой
страницы почти чёрный, замерено. До этой правки проверка считала кнопку под затемнением
вторым пятном.

Проверка от противного на новой проверке диалогов: со старым `--ok` она падает ровно тем
нарушением, которое нашёл рецензент (`color-contrast serious` на подписи «лучшее по итогу»),
с новым — проходит.

## Второй круг проверки ветки sales-ready (2026-09-23)

Правки первого круга ушли на повторную проверку в новую сессию. Семь заявленных правок из восьми
рецензент подтвердил опытом: справка за пять персон и девять статей — 45 сочетаний из 45 без
единого отказа и без перестраховки; контраст пересчитан вручную; со старым цветом новая проверка
диалогов падает. Заблокировали слияние два новых HIGH — в том же сравнении.

| Уровень | Находка                                                                                                                                                                                          | Как закрыто                                                                                                                                            |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| HIGH    | «У Exon срок и формат внедрения не указаны» — на 1exon.ru написано «Процесс внедрения занимает 1 месяц» и «посредством интернет-сервиса (SaaS) или установкой в ИТ-инфраструктуру (on-premises)» | строка переписана по тексту страницы; заодно исправлено про офлайн — он на сайте заявлен                                                               |
| HIGH    | «У ЦУС срок и формат внедрения не указаны» — на tsus.ru «от 1 до 2 месяцев», «Внедрите ЦУС за 1 месяц на одном объекте», «Выберите модель ЦУС: Enterprise или SaaS»                              | строка переписана по тексту страницы                                                                                                                   |
| MEDIUM  | Сужение правила одного оранжевого по `role=dialog` ловило и всплывающие подсказки Radix: при открытой подсказке проверка переставала видеть страницу целиком                                     | сужения нет вовсе: элемент считается пятном, только если виден глазу — проверяется перекрытие в его собственной точке. Замена сужения проверена зондом |
| MEDIUM  | Отметка коммита в отчёте обхода снималась при сборке сводки: пересборка через неделю проставила бы сегодняшний коммит прогону недельной давности                                                 | отметку и дату ставит сам обход и кладёт в отчёт роли; сводка их читает и говорит, если код с тех пор менялся                                          |
| LOW     | Проверка «у раздела есть роль с доступом» холостая: у руководителя доступ ко всем разделам                                                                                                       | заменена на проверку, которая может упасть: у статьи должен быть переход, открытый той роли, для которой она написана. Доказано поломкой               |
| LOW     | В «Коротко» STATE.md висели цифры от 20.09 рядом с новыми                                                                                                                                        | блок обновлён по последнему прогону                                                                                                                    |

Причина обоих HIGH одна: утверждение об отсутствии проверялось пересказом страницы, а не её
текстом. У 1exon.ru полтора мегабайта разметки, нужное лежало ниже пересказа. Правило записано
поправкой к [ADR-020](docs/adr/ADR-020-name-and-alternatives.md): отсутствие проверяется поиском
по всему тексту страницы, утверждение об отсутствии называет страницу, и это держит тест.

Остаётся открытым (LOW, не блокирует): поиск по справке, отбрасывающий выбранную статью, меняет
содержимое, но не адрес; `/help?article=` с несуществующим именем не нормализуется; всплывающая
подсказка экрана — `role=dialog` без имени, axe считает это нарушением уровня serious, но оно
живёт и в `main`, ветка его не добавляла.

## Третий круг проверки ветки sales-ready (2026-09-25)

Блокирующих находок нет: оба HIGH второго круга рецензент подтвердил закрытыми, скачивая страницы
конкурентов целиком, а не пересказом. Проверено заново **все двадцать** утверждений о чужих
системах — фактических ошибок не осталось; справка чиста на всех 45 сочетаниях ролей и статей;
правило одного оранжевого сошлось на восьми зондах в обе стороны. Закрыто по его находкам:

| Уровень | Находка                                                                                                                                                                                                                                   | Как закрыто                                                                             |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| MEDIUM  | Сторож «утверждение об отсутствии называет страницу» знал четыре словоформы и пропускал обычные отрицания — «нет», «отсутствует», «не предусмотрен», «не упоминается». Собственная строка ветки про Билдокс написана как раз словом «нет» | выражение расширено, поймано зондом                                                     |
| MEDIUM  | «Упоминается только в обзорах партнёров» — положительное утверждение о чужой системе, которого нет на указанной странице                                                                                                                  | фраза убрана: осталось проверяемое «на сайте мобильное приложение не заявлено»          |
| MEDIUM  | Сводка обхода предупреждала об устаревании только когда отметка одна: при разных отметках у ролей молчала, без отметки — выдумывала расхождение                                                                                           | три случая разведены и названы словами; проверено четырьмя подложенными наборами данных |
| MEDIUM  | Отчёт обхода в ветке нёс отметку предыдущего коммита                                                                                                                                                                                      | обход перезапущен на финальном коде, отчёт пересобран                                   |
| LOW     | «На страницах обоих решений» ссылалось на одну; название источника не совпадало с названием страницы; в перечне модулей Exon пропущен СУИД; комментарий описывал не тот инвариант, что держит тест                                        | исправлено по месту                                                                     |

**Главное, что рецензент назвал непокрытым, закрыто командой, а не обещанием.** Тексты сравнения
ничем не сверялись с источниками: тест требовал ссылку и дату, но страницу не открывал никогда —
и три круга подряд уходили на то, что делается скриптом. Теперь у каждого утверждения есть слова,
на которых оно держится (`probe`), а `bun run check:sources` скачивает страницы целиком и
проверяет: что заявлено — на месте, чего нет — по-прежнему нет. 54 проверки по шести страницам.
Команда падает и когда сверке больше 90 дней. В конвейер не входит: нужна сеть, и чужой сайт
не должен ронять наш выпуск.

Остаётся открытым (LOW): проверка «у статьи есть переход, открытый её роли» требует хотя бы
одного открытого перехода — статья про закупки с дописанным прорабом её не уронит, потому что
переход в материалы прорабу открыт. Чтобы закрыть по-настоящему, статье нужен признак главного
экрана, которого сейчас нет.
...
| ~~`withDefaultSession` выдаёт сессию руководителя любому GET без сессии~~ — снято: вход по телефону и коду, `requireLogin` (ADR-021)                                                                                   | сделано   | Очередь минимальной внедряемости, 25.09.2026                     |
...
## Очередь минимальной внедряемости (25–26.09.2026)

Ветка `feature/deployable`. Цель: первая неделя из [docs/IMPLEMENTATION.md](docs/IMPLEMENTATION.md)
физически проходится у реального заказчика. Решения: [ADR-021](docs/adr/ADR-021-login-and-invites.md)
(вход, приглашения, первый руководитель), [ADR-022](docs/adr/ADR-022-manual-field-report.md) (отчёт
руками), [ADR-023](docs/adr/ADR-023-catalogs-and-import.md) (справочники и загрузка из Excel),
[ADR-024](docs/adr/ADR-024-zones-from-screen.md) (захватки), [ADR-025](docs/adr/ADR-025-positions-by-hand.md)
(позиции), [ADR-013](docs/adr/ADR-013-tests-and-pipeline.md) (порог бандла по контуру).

Закрыто по пунктам задания: честность текстов в обоих контурах с тестом-сторожем; заметка про
Telegram первым абзацем; вход сотрудника по телефону и коду с серверными сессиями; приглашения и
экран сотрудников; отчёт с площадки руками; справочники с экрана с загрузкой номенклатуры из
Excel и журналом; корень адреса по роли; счётчики как фильтры; ADR-009 и ADR-016 в main с
проверкой ссылок; план внедрения по факту.

Три обрыва, которых в задании не было и которые нашла проверка на чистой базе:

| Обрыв                              | Как закрыт                                                                      |
| ---------------------------------- | ------------------------------------------------------------------------------- |
| Первого сотрудника завести нечем   | команда `bun run db:owner` с одноразовой ссылкой на 72 часа (ADR-021, поправка) |
| Позиции спецификации завести нечем | заведение руками и загрузка таблицей из Excel (ADR-025)                         |
| Захватки объекта завести нечем     | создание и правка на вкладке «Ход работ», изменение в историю объекта (ADR-024) |

Дефекты, найденные попутно и закрытые тем же заходом:

| Уровень | Что                                                                                                     |
| ------- | ------------------------------------------------------------------------------------------------------- |
| HIGH    | `AUTH_CHANNEL` по умолчанию показывал код входа на экране: набрал чужой телефон — вошёл чужими правами  |
| HIGH    | Приёмка отчёта не доходила до факта захватки: «Ход работ» после приёмки не двигался                     |
| HIGH    | Самозакрывающиеся ячейки .xlsx сдвигали значения соседей — выгрузка из 1С полна таких ячеек             |
| MEDIUM  | Пустая карточка объекта закрывала «Ход работ» и обещала разбор документов и бота Telegram               |
| MEDIUM  | Окно загрузки обещало «Загрузить и разобрать» и спорило само с собой; справка вела несуществующим путём |
| MEDIUM  | Мастер запроса зависел от фикстур: на чистой базе шаг «Письмо» не заполнялся, запрос падал с 500        |
| MEDIUM  | Листы документа и очередь разбора выдумывались: «в обработке» без конца, «Лист 7» по размеру файла      |
| LOW     | «Выключить доступ» срабатывало без подтверждения; загрузка заводила материал, который форма не сохранит |

Остаётся:

| Что                                                                                            | Оценка   | Почему не сделано сейчас                                                |
| ---------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------- |
| Разбор документов: таблицы из PDF, DOCX, XLSX в позиции                                        | отдельно | Основная работа следующей очереди; сегодня спецификацию грузят таблицей |
| Приём отчётов из Telegram и расшифровка голоса                                                 | отдельно | Очередь R3–R4; сегодня отчёт заводится кнопкой                          |
| Экран шаблонов писем: сохранить свой шаблон запроса                                            | 3 ч      | Встроенный шаблон правится в мастере на один запрос                     |
| Разделить «выполнено до начала учёта» и принятые объёмы двумя столбцами                        | 4 ч      | Тянет миграцию и пересчёт «Хода работ» и дашборда                       |
| Экран «Захватки и объёмы» (`/zones`) остаётся заготовкой: захватки правятся на вкладке объекта | 4 ч      | Дерево уровней с объёмами по видам работ — отдельная работа             |
| Право ПТО на захватки: сейчас их правит только руководитель                                    | решение  | Нужно решение владельца, матрица прав одна на всё                       |
| Сквозной тест первой недели на чистой базе в конвейере                                         | 4 ч      | Нужна оснастка с PostgreSQL и рабочей сборкой; сейчас сценарий ручной   |

## Аудит кода перед выпуском (26.09.2026)

Что проверялось: заметки вида TODO и FIXME, приведения к `any` и подавления проверок, забытые
`console.log`, размер модулей, безопасность входа и куки, склейка SQL, обработка отказов,
двойная отправка форм, уборка служебных таблиц, формат чисел.

Чисто: ни одного TODO/FIXME, ни одного `as any` и `@ts-expect-error` вне сгенерированного
`routeTree.gen.ts`, забытых `console.log` нет, куки сессии — `httpOnly`, `sameSite=lax` и
`secure` на https, SQL склеивается только из имён контрактов, отказы портов переводятся в коды
ответа и не выносят наружу ни стек, ни схему, двойная отправка закрыта состоянием кнопки.

Закрыто этим заходом:

| Уровень | Находка                                                                                               | Как закрыто                                                                   |
| ------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| HIGH    | Неожиданный отказ (500, 400, обрыв связи) человеку не показывался: кнопка молча не срабатывала        | общий обработчик мутаций говорит словами; тест держит тексты и шаблоны вместе |
| MEDIUM  | `auth_codes` не убиралась: строка на каждую попытку входа, хранятся хэши, которые ничего не открывают | использованные и просроченные коды старше суток удаляются при выдаче нового   |
| LOW     | План захватки в уведомлении печатался без разрядов: «1240» вместо «1 240»                             | `fmtNum`, как везде                                                           |

Остаётся (не трогал: риск больше пользы перед выпуском):

| Что                                                                                         | Оценка | Почему                                                                                                        |
| ------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------- |
| `src/adapters/demo/actions.ts` — 2 251 строка: все записи предметной области в одном модуле | 6 ч    | Разделить по областям (позиции, закупка, поставки, площадка, справочники); нужен свой заход и прогон паритета |
| `src/routes/projects/$id/documents/$docId.tsx` — 1 104 строки                               | 4 ч    | Вынести панель позиций и клавиатуру экрана в свои модули                                                      |
| Общий обработчик отказов проверен разбором текстов, а не прогоном                           | 2 ч    | Нужен сквозной тест рабочего контура: 500 в демо-контуре нечем вызвать                                        |
