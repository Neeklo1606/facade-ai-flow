...
- **P2-3.** «Отменить» — мутация `undoReview`: клиент называет позицию, отменяемое решение и решение до него,
  ...
  («Действие отменено»). Позицию, которую успели изменить иначе, и объединение, цель которого уже объединена
  дальше, отмена не трогает и сообщает об этом. Мутации проверки перечитывают список только после последней
