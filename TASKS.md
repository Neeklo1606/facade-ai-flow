...
- **P2-3.** «Отменить» — мутация `restoreReview`: возвращает поля проверки, но запись в журнале изменений
  позиции остаётся. Отражать отмену в журнале — задача адаптера БД (фаза 3).
...
- **P2-6.** Отложенные события симулятора (стадии распознавания, ответы поставщиков, заказ, отгрузка)
  ...
  уведомлением через мост `api.demo.onEvent`, сам адаптер интерфейс не трогает.
