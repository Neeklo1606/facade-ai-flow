...
| 3 | Выполнена в ветке `phase-3-server`: P3-1…P3-5 и хвосты проверки фазы 2, см. «Итоги фазы 3». Адаптер PostgreSQL в таблицу фазы не входил — нужен выбор хостинга БД |
| 4–5 | Не начаты |
...
- **P3-4.** Таблица `extraction_jobs` (контракты, схема БД, фикстуры; DDL и данные проверены на PostgreSQL 16).
  ...
  В режиме `server` стадии видны без событий демо. Две задачи «Короны» в фикстурах стоят в очереди и
  распознавании — симулятор их не двигает, чтобы цифры сценария показа не менялись.
